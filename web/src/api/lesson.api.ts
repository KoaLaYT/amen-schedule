...
export class LessonApi {
  ...
  static async summary(dateRange: [string, string]): Promise<LessonVo[]> {
    return (
      (await RestApi.get(
        `/lesson/summary?startDate=${dateRange[0]}&endDate=${dateRange[1]}`
      )) ?? []
    );
  }
  /**
  ...
  static async update(date: string, lesson: Lesson) {
    return await RestApi.put<LessonVo>(`/lesson/${lesson.lessonId}`, <
      LessonDto
    >{
      studentId: lesson.studentId,
      taughtDate: date,
   ...
   * 复制课程.
   */
  static async copy(originDate: string, targetDate: string) {
    await RestApi.post(
      `/lesson/copy?originDate=${originDate}&targetDate=${targetDate}`,
      {}
    );
  }
}
